...
$$
\begin{cases}
 \partial_t w + \sup_{a \in A} \big[ \mathcal{L}^a w(t,x) + f(t,x,a) \big] \le 0,
 \quad (t,x) \in [0,T) \times \mathbb{R}^n, \\
 w(T,x) \ge g(x), \quad x \in \mathbb{R}^n.
\end{cases}
$$

Then $w \geq v$ on $[0,T] \times \mathbb{R}^n$.

(2) Suppose further that $w(T,\cdot)=g$ and there exists a measurable map $(t,x) \mapsto \hat{\alpha}(t,x)$ such that

$$
\begin{align}
\partial_t w + \sup_{a \in A} \big[ \mathcal{L}^a w(t,x) + f(t,x,a) \big]
=
\partial_t w + \mathcal{L}^{\hat{\alpha}(t,x)} w(t,x)
+ f(t,x,\hat{\alpha}(t,x)),
\end{align}
$$
  ...
  $$
  \begin{align}
      d X_s = b(X_s, \hat{\alpha}(s,X_s)) \, ds
      + \sigma(X_s, \hat{\alpha}(s,X_s)) \, dW_s,
    \end{align}
    $$
  admits a unique solution  $\hat{X}_s^{t,x}$, with $\hat{\alpha}(s,X_s^{t,x}))_{t \le s \le T} \in \mathcal{A}(t,x)$.

Then $w = v$ on $[0,T] \times \mathbb{R}^n$ and
$\hat{\alpha}$ is an optimal Markovian control.
...
We denote by $\mathbb{S}^2([0,T];\mathbb{R})$ the set of $\mathbb{R}$-valued progressively measurable processes
$Y=(Y_t)_{0 \leq t \leq T}$ such that
$\mathbb{E}\big[ \sup_{0 \leq t \leq T} |Y_t|^2 \big] < \infty$,
and by $\mathbb{H}^2([0,T];\mathbb{R}^n)$ the set of $\mathbb{R}^n$-valued progressively measurable processes
$Z=(Z_t)_{0 \leq t \leq T}$ such that
$\mathbb{E}\big[ \int_0^T |Z_t|^2 \, dt \big] < \infty$.

We are given a pair $(\xi,f)$ called terminal condition and generator (or driver) satisfying:
...
- **(B)** $f : \Omega \times [0,T] \times \mathbb{R} \times \mathbb{R}^d \to \mathbb{R}$ such that
  ...
  - $f$ satisfies a uniform Lipschitz condition in $(y,z)$, i.e. there exists a constant $C_f$ such that
    ...
    \le C_f\big(|y_1-y_2| + |z_1-z_2|\big),
    \quad dt \otimes d\mathbb{P}\text{-a.e.}
    $$
...
\end{align}
$$ 
We now show that for a suitable choice of $\beta$, the  mapping $\Phi$ is  well defined on $\mathbb{S}^2([0,T];\mathbb{R}) \times \mathbb{H}^2([0,T];\mathbb{R}^n)$ into $\mathbb{S}^2([0,T];\mathbb{R}) \times \mathbb{H}^2([0,T];\mathbb{R}^n)$ as $(Y,Z) = \Phi(U,V)$. Formally, defining the martingale process $M=(M_t)_{0 \leq t \leq T}$ as
$$
\begin{align}
M_t = \mathbb{E} \Big[ \xi + \int_{0}^{T} f(s,U_s,V_s)| \mathcal{F}_t \Big],
\end{align}
$$
from the Itô's martingale decomposition theorem, we got the existence of a process $Z=(Z_t)_{0 \leq t \leq T} \in \mathbb{H}^2([0,T];\mathbb{R}^n)$ such that
$$
\begin{align}
