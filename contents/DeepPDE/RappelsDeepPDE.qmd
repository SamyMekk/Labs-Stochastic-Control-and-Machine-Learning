...
:::{#thm-DPPprinciple name="Dynamic Programming Principle."} 
  Let $(t,x) \in [0,T] \times \mathbb{R}^d$. Then, we have
$$
\begin{align}
...
$$
\begin{align}
\mathcal{L}^{a}v = b(x,a) \cdot D_x v + \frac{1}{2} \sigma \sigma^{\top} : D_x^2 v
\end{align},
$$
where $\cdot$ denotes the scalar product between vectors on $\mathbb{R}^d$ and $:$ denotes the scalar prodcutes between matrix, i.e., $A:B= \text{Tr}(A^{\top} B)$. Substituting into @eq-inequality-dynamicprog, we get
$$
\begin{align}
...
$$
\begin{align}
 \partial_t v(t,x) + \underset{a \in A}{\text{ sup }} \big[ \mathcal{L}^{a} v(t,x) + f(t,x,a) \big ] =0 , \quad \forall (t,x) \in [0,T) \times \mathbb{R}^d,
\end{align}
$$ {#eq-PDE-dynamicprog}
...
$$
\begin{align}
\partial_t v(t,x) + H \big(t,x, D_x v(t,x) , D^2_x (t,x) \big) = 0, \quad \forall (t,x) \in [0,T) \times \mathbb{R}^d,
\end{align}
$$ {#eq-PDE-dynamicprog2}
where for $(t,x,p,M) \in [0,T] \times \mathbb{R}^d \times \mathbb{R}^d \times \mathcal{S}_n$, we denote
...
\end{align}
$$
The map $H$ is called the Hamiltonian of the associated control problem and equation @eq-PDE-dynamicprog2 is called dynamic programming equation (DPP) equation or Hamilton-Jacobi-Bellman (HJB) equation. The terminal condition to this PDE is given by

$$
\begin{align}
v(T,x) = g(x), \quad \forall x \in \mathbb{R}^d,
\end{align}
$$
...
\begin{align}
\begin{cases}
\partial_t v(t,x) + \mathcal{L}^{a_0} v(t,x) &=  f(t,x,a_0), \quad \forall (t,x) \in [0,T) \times \mathbb{R}^d. \\
v(T,x) &= g(x), \quad \forall x \in \mathbb{R}^d.
\end{cases}
\end{align}
$$

- The optimality argument of the DPP suggests that if one can find a measurable map $ [0,T] \times \mathbb{R}^d \ni (t,x) \mapsto \alpha^{\star}(t,x) \in A$ such that
...
\begin{align}
\begin{cases}
\partial_t v(t,x) +  \mathcal{L}^{\alpha^{\star}(t,x)} v(t,x) +  f(t,x, \alpha^{\star}(t,x)) &=0, \quad \forall (t,x) \in [0,T) \in \mathbb{R}^d \\
...
:::{#thm-verificationtheorem name="Verification theorem."} 
  Let $w$ be a function in $\mathcal{C}^{1,2}([0,T) \times \mathbb{R}^d) \cap \mathcal{C}^0([0,T] \times \mathbb{R}^d)$, satisfying a growth quadratic growth condition, i.e.,  there exists a constant $C \geq 0$ such that
  
$$
\begin{align}
|w(t,x)| \leq C \big( 1+ |x|^2 \big), \quad \forall (t,x) \in [0,T] \times \mathbb{R}^d.
\end{align}
$$
- Suppose that 
...
\begin{align}
\begin{cases}
 \partial_t w + \underset{a \in A}{\text{ sup }} \big[ \mathcal{L}^a w(t,x) + f(t,x,a) \big] &\leq 0, \quad (t,x) \in [0,T) \times \mathbb{R}^d, \\
 w(T,x) &\geq g(x), \quad x \in \mathbb{R}^d.
\end{cases}
\end{align}
$$
Then $w \geq v$ on $[0,T] \times \mathbb{R}^d$.

- Suppose further $w(T,\cdot)=g$ and there exists a measurable map $[0,T] \times \mathbb{R}^d \ni (t,x) \mapsto \hat{\alpha}(t,x) \in A$ such that
$$
\begin{align}
...
$$
admits a unique solution, denoted by $\hat{X}_s^{t,x}$ and the process $\big(\hat{\alpha}(s,X_s^{t,x}) \big)_{t \leq s \leq T} \in \mathcal{A}(t,x)$. Then, we have $w=v$ on $[0,T] \times \mathbb{R}^d$ and $\hat{\alpha}$ is an optimal Markovian control. 
:::
...
::: {#alg-description .algorithm}
<span class="para-subtitle">Deep Galerkin Algorithm : </span>
...
::: {#alg-description .algorithm}
<span class="para-subtitle">Deep BSDE Algorithm : </span>
...
::: {#alg-description .algorithm}
<span class="para-subtitle">Deep BDP Algorithm : </span>
